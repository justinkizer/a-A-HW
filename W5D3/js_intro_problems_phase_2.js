...
function titleize (array, callback) {
  const titledResult = array.map(function (el) { return `Mx. ${el} Jingleheimer Schmidt`;});
  callback(titledResult);
}
...
Elephant.prototype.play = function () {
  const trick = this.tricksArray[Math.floor(Math.random() * (this.tricksArray.length - 0) + 0)];
...
function dinerBreakfast () {
  ...
  console.log(order);
  return function (extraOrder) {
    order = `${order.slice(0, order.length - 9)} and ${extraOrder}, please.`;
    console.log(order);
  };
}
